	...
	if (numSlots <= 0) {
		...
		if (rv != CKR_OK)
		{
			...
			*/
			...
			// generate RSA key
			rv = pFuncList->C_GenerateKeyPair(sessionHandle, &mechanismRSAKey, pPubKeyAttribtueTemplate, 7, pPriKeyAttribtueTemplate, 6, &hPubKeyObject, &hPriKeyObject);
			if (rv != CKR_OK)
			{
			...
			// template of CKA_LABEL to be change label.
			CK_ATTRIBUTE template[] = { CKA_LABEL, newlabel, strlen(newlabel) };
	
 
			//hPriKeyObject is a handle to the private key object for which the attribute value is being set.
			rv = pFuncList->C_SetAttributeValue(sessionHandle, hPriKeyObject, template, 1);
			if(rv != CKR_OK)
			{
				printf("C_SetAttributeValue() error --- (rv : %x)", rv);				
			}
			else
			{
				printf("C_SetAttributeValue Seccess -> %s\n", newlabel);
				hPriKeyObject = '\0';
			}

			//hPublicKeyObject is a handle to the PublicKey object for which the attribute value is being set.
			rv = pFuncList->C_SetAttributeValue(sessionHandle, hPubKeyObject, template, 1);
			if(rv != CKR_OK)
			{
				printf("C_SetAttributeValue() error --- (rv : %x)", rv);
			}
