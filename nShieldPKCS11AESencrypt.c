	...
	if (numSlots <= 0) {
		...
		if (rv != CKR_OK)
		{
			...
			CK_ATTRIBUTE	pAESKeyAttribtueTemplate[] = {
				...
				{ CKA_TOKEN, &falseValue, sizeof(falseValue) },       /* if true and CKNFAST_OVERRIDE_SECURITY_ASSURANCES=tokenkeys or all in cknfastrc, key will be generated and key blob file will be created    *
			...
			int nSize;
			printf("Encrypt Data : ");
			for(nSize=0; nSize < iDataSize ; nSize++)	
					printf("%02X", OutputBuffer[nSize]);
				printf("\n");
