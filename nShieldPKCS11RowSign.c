	...
	if (numSlots <= 0) {
		...
		if (rv != CKR_OK)
		{
			...
			*/
			...
			CK_MECHANISM verify_mechanism;

			CK_BYTE pDigest[32];
			CK_MECHANISM mechanism_sha = { CKM_SHA256, NULL_PTR, 0 };
			...
			rv = pFuncList->C_DigestInit(sessionHandle, &mechanism_sha);
			if (rv != CKR_OK)
			{
				printf("C_DigestInit failed. --- (rv : %x)\n", rv);
			}

			rv = pFuncList->C_Digest(sessionHandle, plaintext, ulPlaintextLen , pDigest, &ulDigestLen);
			if(rv != CKR_OK)
			{
				printf("C_DigestInit failed. --- (rv : %x)\n", rv);
			}
			...
			printf("Hash Value -> ");
			for(nSize=0; nSize < ulDigestLen ; nSize++)	
				printf("%02X", pDigest[nSize]);
			printf("\n");

			sign_mechanism.mechanism = CKM_RSA_9796;
			...
			rv = pFuncList->C_Sign(sessionHandle, pDigest, ulDigestLen, signature, &ulSignatureLen);
			if(rv != CKR_OK)
			{
			...
			verify_mechanism.mechanism = CKM_RSA_9796;
			verify_mechanism.pParameter = NULL_PTR;
			verify_mechanism.ulParameterLen = 0;

			rv = pFuncList->C_VerifyInit(sessionHandle, &verify_mechanism, hPubKeyObject);
			if(rv != CKR_OK)
			{
				printf("Failed to call C_VerifyInit. --- (rv : %x)", rv);
				exit(0);
			}

			rv = pFuncList->C_Verify(sessionHandle, pDigest, ulDigestLen, signature, ulSignatureLen);
			if(rv != CKR_OK)
			{
				if(rv == CKR_SIGNATURE_INVALID)
				{
					printf("The provided signature/MAC is invalid.\n");
					exit(0);
				}
				else if(rv == CKR_SIGNATURE_LEN_RANGE)
				{
					printf("The provided signature/MAC can be seen to be invalid solely on the basis of its length.\n");
					exit(0);
				}
				else
				{
					printf("Failed to call C_Verify. --- (rv : %x)", rv);
					exit(0);
				}
			}
			else
