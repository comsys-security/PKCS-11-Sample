...
// Notes: 
//		This program is used for demonstrating random data generation by nShield
...
int main(int argc, char **argv[])
{
	...
	if (numSlots > 0) {
		...
		printf("There are total %i slots in the system.\n", numSlots);
		printf("Accelator, Softcard, OCS slot can be used.\n");
		...
		if (rv != CKR_OK)
		{
			...
			// perform crypto operation
				...
				// define max random data size
				int RandomDataLength = 1024;
				CK_BYTE *pRandomData;
				int a = 0;
				// prompt user to input size of random data
				printf("Please enter data size to be generated (in byte): ");
				scanf("%d", &RandomDataLength);
				printf("\n");

				// allocate buffer for random data
				pRandomData = (CK_BYTE *)calloc(sizeof(CK_BYTE), RandomDataLength);
				
				// use nShield to generate random data
				rv = pFuncList->C_GenerateRandom(sessionHandle, pRandomData, RandomDataLength);
				if (rv != CKR_OK)
				{
					...
					// output the first 8 bytes of random data in hexadecimal form
					printf("\nRandom Data generated by nShield (in hex): ");
					for (a = 0; a < RandomDataLength; a++)
						printf("%02X", pRandomData[a]);
					printf("\n");
				}
				
				// free buffer for random data
				free(pRandomData);
