...
// This is sample application running in Microsoft Windows.
		...
		if (rv != CKR_OK)
		{
			...
			*/
			...
			CK_ATTRIBUTE	pPriKeyAttribtueTemplate[] = {
				...
				{ CKA_LABEL, "RSA Private Key", sizeof("RSA rivate Key") },    // key label; must include in key generation
			...
			rv = pFuncList->C_DestroyObject(sessionHandle, hPriKeyObject);
			if(rv != CKR_OK)
			{
				printf("DestroyObject() error --- (rv : %x)", rv);
			}
			else
			{
				printf("Delete KEY Seccess.\n");
				hPriKeyObject = '\0';
			}

			rv = pFuncList->C_DestroyObject(sessionHandle, hPubKeyObject);
			if(rv != CKR_OK)
			{
				printf("DestroyObject() error --- (rv : %x)", rv);
			}
